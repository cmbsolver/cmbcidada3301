            ...
            public async Task Handle(Command notification, CancellationToken cancellationToken)
            {
                ...
                while (dontExit)
                {
                    ...
                    var selecttion = AnsiConsole.Prompt(
                        ...
                        .AddChoices(new[] {
                        "1: Flush Output Directories",
                        "2: Flush Input Directories",
                        "3: Check Directories",
                        "99: Previous Menu",
                        }));
                    ...
                    {
                        case "1":
                            await _mediator.Publish(new FlushOutputDirectory.Command());
                            break;

                        case "2":
                            await _mediator.Publish(new FlushInputDirectory.Command());
                            break;

                        case "3":
                            await _mediator.Publish(new CheckForOutputDirectory.Command());
                            break;
