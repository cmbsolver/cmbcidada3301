...
public class TakeAndSkipText
{
    ...
    public class Handler : IRequestHandler<Command, string>
    {
        ...
        public async Task<string> Handle(Command request, CancellationToken cancellationToken)
        {
            ...
            {
                skipCounter += request.Skip;
                
                if (skipCounter > text.Length - 1)
                {
                    skipCounter = 0;
                    sb.Append(text.Substring(skipCounter + 1, takeCounter));
                    iterCounter++;
                ...
                {
                    sb.Append(text.Substring(skipCounter, takeCounter));
                    skipCounter += request.Take;
                }
            }
            
            var rtext = await _mediator.Send(new TransposeRuneToLatin.Command(sb.ToString()), cancellationToken);
            rtext = await _mediator.Send(new FixUpControlChars.Command(rtext), cancellationToken);
            return rtext;
        }
    }
}